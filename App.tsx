...
function App() {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (loginStep === 1) {
      setLoginStep(2);
      // GTM track step 1
      window.dataLayer.push({ event: 'login_step_complete', step: 1, method: 'user_id' });
    } else {
  ...
  const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
    if (!isLoggedIn) {
      return <div className="min-h-[60vh] flex flex-col items-center justify-center p-4">
        ...
        <Button onClick={() => setIsLoginModalOpen(true)}>Login Now</Button>
      </div>;
    }
    return <>{children}</>;
  };
      ...
      <div className="flex flex-col min-h-screen font-sans text-gray-900">
        <Navbar 
          ...
          onLoginClick={() => {
            ...
            window.dataLayer.push({ event: 'login_modal_open', source: 'navbar' });
          }} 
        />
        ...
        <main className="flex-grow">
          <Routes>
            ...
            <Route path="/bill-pay" element={<ProtectedRoute><BillPay /></ProtectedRoute>} />
        ...
        <Modal 
          ...
          onClose={() => {
            setIsLoginModalOpen(false);
            setLoginStep(1);
            window.dataLayer.push({ event: 'login_modal_close' });
          }}
